          ...
          {plans.map((plan, index) => (
            <Card key={index} className="bg-white/10 backdrop-blur-md border border-sopfuel-gray text-white shadow-lg">
              ...
              <CardHeader className="text-center">
                <CardTitle className="text-white font-montserrat text-2xl">{plan.name}</CardTitle>
                <div className="space-y-2">
                  <div className="text-4xl font-bold text-white">
                    {plan.price}
                  </div>
                  <div className="text-[#EEEEEE] font-inter">
                    {plan.period}
                  </div>
                </div>
                <CardDescription className="text-[#EEEEEE] font-inter">{plan.description}</CardDescription>
              </CardHeader>
                ...
                <ul className="space-y-3">
                  {plan.features.map((feature, featureIndex) => (
                    <li key={featureIndex} className="flex items-center space-x-3">
                      ...
                      <span className="text-[#EEEEEE] font-inter">{feature}</span>
                    </li>
                  ))}
                </ul>
                <Button 
                  className={plan.popular ? "w-full btn-premium font-montserrat text-white text-lg shadow-xl" : "w-full border border-brand-blue/70 bg-white/10 text-white font-montserrat hover:bg-brand-blue/80 hover:text-white transition"}
